...
for iPl = 1:dimPl
    ...
    title(ax1, ['distr.: ' dist ', \mu_X = ' num2str(pdRe.mu,'%5.4f') ', ' ...
         ' \sigma_X = ' num2str(pdRe.sigma,'%5.4f')]);
    ...
    title(ax2, ['\mu_Y = ' num2str(pdIm.mu,'%5.4f') ', ' ...
         ' \sigma_Y = ' num2str(pdIm.sigma,'%5.4f') ', Pl.' iPlStr]);
    ylabel('% of total data');
    xlabel('Y');
    grid on;
    hold off;
    ...
    if figSaveTitle == 1
        savefig(strcat(filenameFig, figNameTitleXY, iPlStr, '.fig')); % fig format
        saveas(gcf, strcat(filenameFig, figNameTitleXY, iPlStr, '.jpg')); % jpg format;
